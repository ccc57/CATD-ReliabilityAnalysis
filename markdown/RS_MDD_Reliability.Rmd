...
all_stats$Chance <- c("Chance Discriminability","Chance Discriminability","Chance Discriminability")
...
ggplot(all_stats,aes(x=rownames(all_stats),y=FI,fill=rownames(all_stats)))+
  ...
  theme_classic() + theme(text = element_text(size=20)) + guides(linetype="test") +
...
ggplot(all_stats,aes(x=rownames(all_stats),y=Discr,fill=rownames(all_stats)))+ 
  ...
  geom_segment(aes(x=c(.75,2.75,1.75),
                   ...
                   yend = Discr_null,linetype=Chance),size=1) + 
  ...
  scale_color_manual(values = c("#d45757", "#FECDAA","#6785E3"),aesthetics = "fill") + 
  theme_classic() + theme(legend.justification=c(1,1),legend.position=c(1,1),text = element_text(size=20))

# Sig indicators on bar charts
