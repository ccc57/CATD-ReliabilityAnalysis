...
get_FI <- function(ses1, ses2, continuous = FALSE, null = FALSE){
    ...
    corr <- rcorr(t(rbind(ses1, ses2)))$r
    diag(corr) <- 0
    d <- dim(corr)[1]
    r1 <- corr[1:(d/2), (d/2 + 1):d]
    r2 <- corr[(d/2+1):d, 1:(d/2)]
    
    if(!continuous){
      if(null){
        r1.null <- r1[sample(d/2),]
        r2.null <- r2[sample(d/2),]
        return(mean(sum(argmax(r1.null) == 1:(d/2))/(d/2), sum(argmax(r2.null) == 1:(d/2))/(d/2)))
      }
      return(mean(sum(argmax(r1) == 1:(d/2))/(d/2), sum(argmax(r2) == 1:(d/2))/(d/2)))
    }
    else{
      return(rowMeans(cbind(diag(r1), diag(r2))))
    }
