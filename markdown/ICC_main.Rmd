...
```{r icc, echo=FALSE}
icc_dat <- read.csv(paste0(rootdir,'data/processed/connectivity_data/icc_dataframe.csv'))

mdd <- icc_dat %>% filter(grepl("v1|v4", session)) %>% group_by(subject) %>% filter(length(session) == 2) %>% filter(grepl("v1", session)) %>% ungroup() %>% filter(Participant_Type2 == "MDD")

hv <- icc_dat %>% filter(grepl("v1|v4", session)) %>% group_by(subject) %>% filter(length(session) == 2) %>% filter(grepl("v1", session)) %>% ungroup() %>% filter(Participant_Type2 == "HV")
